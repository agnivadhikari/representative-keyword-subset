...
import pprint
import logging
logging.basicConfig(filename='myProgramLog-2017-5.txt', level=logging.DEBUG, format=' %(asctime)s - %(levelname)s - %(message)s')
import StringIO
...
def tfidf_process_text_files():
  ...
  for i in range(beginning_range):
    ...
    for filename in filenames:
      ...
      if abstract == 'N/A':
	#print 'paper without abstract: ',filename
    ...
    for filename in filename_abstract_dict:
      ...
      for word in all_words:
        ...
        if re.match(r'^[0-9]', word):
          ...
          continue
        words.append(word)
      filename_wordlist_dict[filename]=words
      words_tf = get_tf(words)
      sorted_words_tf = sorted(words_tf.items(), key=lambda x:x[1], reverse = True)
      filename_tf_dict[filename]=sorted_words_tf
    for filename in filename_tf_dict:
      words_tfidf = {}
      sorted_words_tfidf = {}
      sorted_words_tf = filename_tf_dict[filename]
      for (word,tf) in sorted_words_tf:
        word_presence_in_documents_count = 1
        for filename_wordlist in filename_wordlist_dict:
	  if filename==filename_wordlist:
	    continue
	  words = filename_wordlist_dict[filename_wordlist]
          if word in words:
	    word_presence_in_documents_count+=1
        idf = np.log(size_of_document_corpus/word_presence_in_documents_count)
        tfidf = tf*idf
        #print word, tf, size_of_document_corpus/word_presence_in_documents_count, np.log(size_of_document_corpus/word_presence_in_documents_count), tf*np.log(size_of_document_corpus/word_presence_in_documents_count)
        #print word,tfidf
        words_tfidf[word]=tfidf
      sorted_words_tfidf=sorted(words_tfidf.items(), key=lambda x:x[1], reverse = True)
      top_5_sorted_words_tfidf = sorted_words_tfidf[:5]
      keywords_list = []
      for (word,tfidf) in top_5_sorted_words_tfidf:
        keywords_list.append(word)
      if keywords_list == []:
        continue

      # Populate the filename_keywords_dict 
      filename_tfidf_dict[filename+' '+year_str] = top_5_sorted_words_tfidf
      filename_keywords_dict[filename+' '+year_str] = keywords_list
      logging.debug('filename: '+filename+' '+year_str)
      logging.debug('TFIDF-Keywords: '+str(filename_keywords_dict[filename+' '+year_str]))
  ...
  for k in sorted(keyword_vertices_dict, key=lambda k: len(keyword_vertices_dict[k]), reverse=True): # Sort the dictionary based on length of the vertices list    
    ...
    #keyword_statistics_list.append({"keyword":k, "2017":yearly_keyword_count['2017'],"total":len(keyword_vertices_dict[k]), "priority":priority, "sd":sd})
  ...
  while get_count_conn_keywords_vertices_set_dict(conn_keywords_vertices_set_dict) < total_vertices_count:
    ...
    (conn_keywords_set,conn_keywords_vertices_set,conn_keywords_temp_dict_len_dict,conn_keywords_iteration_len_dict) = update_keywords_gist_set(filename_keywords_dict,keyword_vertices_dict,sorted_on_specific_parameter,conn_keywords_set_dict,conn_keywords_vertices_set_dict,conn_keywords_temp_dict_len_dict,conn_keywords_iteration_len_dict)
  ...
  for index in conn_keywords_set_dict:
    ...
    for keyword in keywords_set:
      #logging.debug('Keyword from keywords_set: '+keyword)
...
def get_keyword_index(keyword, sorted_on_specific_parameter):
  ...
  for item in sorted_on_specific_parameter:
    ...
    if item["keyword"]==keyword:
      break
  return i
def update_keywords_gist_set(filename_keywords_dict,keyword_vertices_dict,sorted_on_specific_parameter,conn_keywords_set_dict,conn_keywords_vertices_set_dict,conn_keywords_temp_dict_len_dict,conn_keywords_iteration_len_dict):
  ...
  while(continue_loop):
	...
	if check_keyword_in_gist_set(keyword, conn_keywords_set_dict) or vertices_set.issubset(conn_vertices_set.union(current_cumulative_vertices_set)):
	  ...
	  print str(len(current_cumulative_vertices_set))
	  continue
  ...
  return (current_cumulative_keywords_set, current_cumulative_vertices_set,conn_keywords_temp_dict_len_dict,conn_keywords_iteration_len_dict)
